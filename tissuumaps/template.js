...
Points2Regions = {
  ...
  parameters: {
    ...
    _pixel_size: {
      label:
        "Pixel size (increase/decrease for coarser/finer regions):",
      type: "number",
      default: 5,
    },
    _pixel_smoothing: {
...
mdl = Points2RegionClass(xy, labels, pixel_size, pixel_smoothing, min_pts_per_pixel, lib_id)
...
Points2Regions.selectStride = function (parameterName) {
  ...
  var moveHandler = function (event) {
    ...
    pixel_size = Math.abs(rectangle.width) / 10
    Points2Regions.set("_pixel_smoothing", 5);
    Points2Regions.set("_pixel_size", pixel_size);
    return;
  };
...
Points2Regions.initPython = function () {
  if (!document.getElementById("pyScript")) {
    ...
    pyconfig.innerHTML = "packages=['scikit-learn','scikit-image','typing-extensions']";
